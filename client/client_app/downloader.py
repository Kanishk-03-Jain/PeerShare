...
def download_from_peer(download_url: str, timeout: int, filename: str, filesize: int, destination: str, method_name: str, save_path: str):
    try:
        # Stream the download so we don't crash RAM on big files
        with requests.get(download_url, params={"name": filename}, stream=True, timeout=timeout) as r:
            r.raise_for_status()
            ...
            with tqdm(total=filesize, unit='B', unit_scale=True, desc=filename) as progress_bar:
...
def download_file_strategy(peer_data: dict, filename: str, filesize: int, destination: str):
